...
bool CDib::create( int _width, int _height, int _bits /* = 24 */ )
{
	...
	m_pBmpInfoHeader->biHeight      = -_height; /*top-down*/
...
{
	if(is_valid())
		return abs(m_pBmpInfoHeader->biHeight);
...
int CDib::get_dib_size() const
{
	if(is_valid())
	{
		...
		return _linesize * abs(m_pBmpInfoHeader->biHeight);
	}
				  ...
				  int _xSrc, int _ySrc ) const
{
	if(is_valid())
	{
		...
		return ::SetDIBitsToDevice(hdc, _xDest, _yDest, _width, _height, \
			_xSrc, abs(m_pBmpInfoHeader->biHeight) - _ySrc - _height, \
			0, abs(m_pBmpInfoHeader->biHeight), m_pData, m_pBmpInfo, DIB_RGB_COLORS);
	}
...
int CDib::bitblt( HDC hdc, int _xDest, int _yDest ) const
{
	if(is_valid())
	{
		...
		return ::SetDIBitsToDevice(hdc, _xDest, _yDest, m_pBmpInfoHeader->biWidth, abs(m_pBmpInfoHeader->biHeight), \
			0, 0, \
			0, abs(m_pBmpInfoHeader->biHeight), m_pData, m_pBmpInfo, DIB_RGB_COLORS);
	}
