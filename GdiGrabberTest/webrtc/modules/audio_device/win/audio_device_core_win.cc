...
bool AudioDeviceWindowsCore::CoreAudioIsSupported()
{
    ...
    //commented by zyx,因为学霸君上层的主线程使用了STA，导致这里会失败。
...
AudioDeviceWindowsCore::~AudioDeviceWindowsCore()
{
	...
	if (NULL != _hCapturePlayStartedEvent)
	{
		CloseHandle(_hCapturePlayStartedEvent);
		_hCapturePlayStartedEvent = NULL;
	}

	if (NULL != _hShutdownCapturePlayEvent)
	{
		CloseHandle(_hShutdownCapturePlayEvent);
		_hShutdownCapturePlayEvent = NULL;
	}
...
int32_t AudioDeviceWindowsCore::InitSpeaker()
{
    ...
    if (_usingOutputDeviceIndex)
    {
        ...
        if (_outputDeviceIndex > (nDevices - 1))
        {
            ...
            //return -1;
			/*Added by wrb, 设备拔出时，有可能导致选择的设备index大于设备的总数，此时用默认的设备*/
			_usingOutputDeviceIndex = false;
        }
    }
...
int32_t AudioDeviceWindowsCore::InitMicrophone()
{
    ...
    if (_usingInputDeviceIndex)
    {
        ...
        if (_inputDeviceIndex > (nDevices - 1))
        {
            WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "current device selection is invalid => unable to initialize");
			/*return -1;*/
			/*Added by wrb，设备插拔时会导致原先选择的index大于设备的总数，所以这时选择默认的设备 */
			_usingInputDeviceIndex = false;	
        }
    }
...
int32_t AudioDeviceWindowsCore::InitPlayout()
{
	...
	SAFE_RELEASE(_ptrCapturePlayoutClient);
	hr = _ptrClientOut->GetService(
							__uuidof(IAudioCaptureClient),
							(void**)&_ptrCapturePlayoutClient);
	EXIT_ON_ERROR(hr);
...
int32_t AudioDeviceWindowsCore::InitRecording()
{
    ...
    if (InitMicrophone() == -1)
    {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "InitMicrophone() failed");
    }
    ...
    if (FAILED(hr))
    {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "Activate Device failed with err 0x%x", hr);
    }
    EXIT_ON_ERROR(hr);
    ...
    if (FAILED(hr))
    {
        WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id, "IAudioClient::GetService failed with err 0x%x", hr);
    }
    EXIT_ON_ERROR(hr);
...
/*added by wrb, for capturing playout*/
int32_t AudioDeviceWindowsCore::StartCapturPlayout()
{
	if (!_playIsInitialized)
	{
		return -1;
...

	{
		...
		// Create thread which will drive the capture playout
		assert(_hCapturePlayThread == NULL);
		_hCapturePlayThread = CreateThread(
			NULL,
			0,
			WSAPICapturePlayThread,
			this,
			0,
			NULL);
		if (_hCapturePlayThread == NULL)
		{
			WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
				"failed to create the capture playout thread");
			return -1;
		}

		// Set thread priority to highest possible.
		SetThreadPriority(_hCapturePlayThread, THREAD_PRIORITY_TIME_CRITICAL);
	}  // critScoped

	DWORD ret = WaitForSingleObject(_hCapturePlayStartedEvent, 1000);
	if (ret != WAIT_OBJECT_0)
	{
		WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
			"capturing playout did not start up properly");
		return -1;
	}

	_capturingPlay = true;
	WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
		"capturing playout audio stream has now started...");

	return 0;
}

int32_t AudioDeviceWindowsCore::StopCapturePlayout()
{
	if (!_playIsInitialized)
	{
		return 0;
...

	{
		...
		// stop the driving thread...
		WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
			"closing down the webrtc_core_audio_capture_playout thread...");
		SetEvent(_hShutdownCapturePlayEvent);
	}  // critScoped

	DWORD ret = WaitForSingleObject(_hCapturePlayThread, 2000);
	if (ret != WAIT_OBJECT_0)
	{
		// the thread did not stop as it should
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
			"failed to close down webrtc_core_audio_capture_playout");
		CloseHandle(_hCapturePlayThread);
		_hCapturePlayThread = NULL;
		_playIsInitialized = false;
		_capturingPlay = false;
		return -1;
...

	{
		CriticalSectionScoped critScoped(&_critSect);
		WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id,
			"webrtc_core_audio_capture_playout is now closed");
...
bool AudioDeviceWindowsCore::CaptureingPlayout() const
{
	return (_capturingPlay);
}
/*end*/
...
/*added by wrb, for capturing playout*/
DWORD WINAPI AudioDeviceWindowsCore::WSAPICapturePlayThread(LPVOID context)
{
	return reinterpret_cast<AudioDeviceWindowsCore*>(context)->
		DoCapturePlayThread();
}
...
DWORD AudioDeviceWindowsCore::DoCapturePlayThread()
{
	...
	HANDLE waitArray[2] = { _hShutdownCapturePlayEvent, _hRenderSamplesReadyEvent};
	HRESULT hr = S_OK;
	HANDLE hMmTask = NULL;
	...
	if (!comInit.succeeded()) {
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
			"failed to initialize COM in capture play thread");
		return 1;
	}

	rtc::SetCurrentThreadName("webrtc_core_audio_capture_playout");
	...
	if (_winSupportAvrt)
	{
		...
		if (hMmTask)
		{
			if (FALSE == _PAvSetMmThreadPriority(hMmTask, AVRT_PRIORITY_CRITICAL))
			{
				WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to boost capture-play-thread using MMCSS");
			}
			WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "capture-play thread is now registered with MMCSS (taskIndex=%d)", taskIndex);
		}
		else
		{
			WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to enable MMCSS on capture play thread (err=%d)", GetLastError());
			_TraceCOMError(GetLastError());
		}
	}
	...
	{
		WEBRTC_TRACE(kTraceError, kTraceAudioDevice, _id,
			"output state has been modified before capture loop starts.");
		return 1;
	}
	hr = _ptrClientOut->GetBufferSize(&bufferLength);
	EXIT_ON_ERROR(hr);
	...
	// for cases when the capture buffer is larger than 10ms.
	//
	const UINT32 syncBufferSize = 2 * (bufferLength * _playAudioFrameSize);
	syncBuffer = new BYTE[syncBufferSize];
	if (syncBuffer == NULL)
	{
		return (DWORD)E_POINTER;
	...
	double endpointBufferSizeMS = 10.0 * ((double)bufferLength / (double)_playBlockSize);
	WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "[CAPT] endpointBufferSizeMS : %3.2f", endpointBufferSizeMS);
	...
	// Set event which will ensure that the calling thread modifies the recording state to true.
	//
	SetEvent(_hCapturePlayStartedEvent);
	...
	while (keepCapturePlay)
	{
		...
		switch (waitResult)
		{
		case WAIT_OBJECT_0 + 0:        // _hShutdownCapturePlayEvent
			keepCapturePlay = false;
			break;
		case WAIT_OBJECT_0 + 1:        // _hRenderSamplesReadyEvent
			break;
		case WAIT_TIMEOUT:            // timeout notification
			WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id, "capture play event timed out after 0.5 seconds");
			goto Exit;
		default:                    // unexpected error
		...
		while (keepCapturePlay)
		{
			...
			// Sanity check to ensure that essential states are not modified
			// during the unlocked period.
			if (_ptrCapturePlayoutClient == NULL || _ptrClientOut == NULL)
			{
				_UnLock();
				WEBRTC_TRACE(kTraceCritical, kTraceAudioDevice, _id,
					"output state has been modified during unlocked period");
			...
			//  Find out how much capture data is available
			//
			hr = _ptrCapturePlayoutClient->GetBuffer(&pData,           // packet which is ready to be read by used
				&framesAvailable, // #frames in the captured packet (can be zero)
				&flags,           // support flags (check)
				&recPos,          // device position of first audio frame in data packet
				&recTime);        // value of performance counter at the time of recording the first audio frame
			...
			if (SUCCEEDED(hr))
			{
				...
				if (pData)
				{
					CopyMemory(&syncBuffer[syncBufIndex*_playAudioFrameSize], pData, framesAvailable*_playAudioFrameSize);
				}
				else
				{
					ZeroMemory(&syncBuffer[syncBufIndex*_playAudioFrameSize], framesAvailable*_playAudioFrameSize);
				}
				assert(syncBufferSize >= (syncBufIndex*_playAudioFrameSize) + framesAvailable*_playAudioFrameSize);

				// Release the capture buffer
				//
				hr = _ptrCapturePlayoutClient->ReleaseBuffer(framesAvailable);
				EXIT_ON_ERROR(hr);
				...
				// Get the current recording and playout delay.
				uint32_t sndCardPlayDelay = (uint32_t)
					(((((UINT64)t1.QuadPart * _perfCounterFactor) - recTime)
					/ 10000) + (10 * syncBufIndex) / _playBlockSize - 10);
				uint32_t sndCardRecDelay =
					static_cast<uint32_t>(_sndCardRecDelay);

				_sndCardPlayDelay = sndCardPlayDelay;

				while (syncBufIndex >= _playBlockSize)
				{
					if (_ptrAudioBuffer)
					{
						_ptrAudioBuffer->SetCapturePlayBuffer((const int8_t*)syncBuffer, _recBlockSize);
						_ptrAudioBuffer->SetVQEData(sndCardPlayDelay,
							sndCardRecDelay,
							0);
						...
						_UnLock();  // release lock while making the callback
						_ptrAudioBuffer->DeliverCapturePlayData();
						_Lock();    // restore the lock
					...
					// store remaining data which was not able to deliver as 10ms segment
					MoveMemory(&syncBuffer[0], &syncBuffer[_playBlockSize*_playAudioFrameSize], (syncBufIndex - _playBlockSize)*_playAudioFrameSize);
					syncBufIndex -= _playBlockSize;
					sndCardPlayDelay -= 10;
				}
...
Exit:
	...
	if (keepCapturePlay)
	{
		...
		// Trigger callback from module process thread
		//_playError = 1;
		WEBRTC_TRACE(kTraceError, kTraceUtility, _id, "kPlayingError message posted: capturing playout thread has ended pre-maturely");
	}
	else
	{
		WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "capturing playout thread is now terminated properly");
	}
