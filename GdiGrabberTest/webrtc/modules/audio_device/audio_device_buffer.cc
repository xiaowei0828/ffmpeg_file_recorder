...
AudioDeviceBuffer::AudioDeviceBuffer() :
	...
	memset(_capturePlayBuffer, 0, kMaxBufferSizeBytes);
}
...
int32_t AudioDeviceBuffer::SetCapturePlayBuffer(const void* audioBuffer, size_t nSamples)
{
	CriticalSectionScoped lock(&_critSect);
	...
	_capturePlaySamples = nSamples;
	_capturePlaySize = _playBytesPerSample * nSamples; // {2,4}*nSamples
	if (_capturePlaySize > kMaxBufferSizeBytes)
	{
		assert(false);
		return -1;
	}
...
int32_t AudioDeviceBuffer::DeliverCapturePlayData()
{
	CriticalSectionScoped lock(&_critSectCb);
	...
	if (_ptrCbAudioTransport == NULL)
	{
		WEBRTC_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to deliver recorded playout data (AudioTransport does not exist)");
		return 0;
	}
