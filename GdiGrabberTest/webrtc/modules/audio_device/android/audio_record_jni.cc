...
AudioRecordJni::AudioRecordJni(AudioManager* audio_manager)
  ...
  JNINativeMethod native_methods[] = {
	  ...
	  {"nativeSetDelayInMs",
	   "(IJ)I",
	   reinterpret_cast<void*>(&webrtc::AudioRecordJni::setDelayInMs)
	  },
	  {"nativeGetDelayInMs",
	   "(J)I",
	   reinterpret_cast<void*>(&webrtc::AudioRecordJni::getDelayInMs)
	  }
  };
...
jint JNICALL AudioRecordJni::setDelayInMs(JNIEnv* env, jobject obj, jint delayMs, jlong nativeAudioRecord) {
	  webrtc::AudioRecordJni* this_object =
	      reinterpret_cast<webrtc::AudioRecordJni*> (nativeAudioRecord);

	  this_object->total_delay_in_milliseconds_ = delayMs;

	return 0;
}

jint JNICALL AudioRecordJni::getDelayInMs(JNIEnv* env, jobject obj, jlong nativeAudioRecord) {
	  webrtc::AudioRecordJni* this_object =
	      reinterpret_cast<webrtc::AudioRecordJni*> (nativeAudioRecord);

	return this_object->total_delay_in_milliseconds_;
}
