...
char* CallStack::toStringSingleLevel(const char* prefix, int32_t level) const
{
...
void CallStack::dump(const char* prefix) const
{
    ...
    for (int i=0; i<int(mCount); i++) {
    	char* item = toStringSingleLevel(prefix, i);

    	__android_log_print(ANDROID_LOG_ERROR, "lqp_test", "%s", item);
...
char* CallStack::toString(const char* prefix) const
{
